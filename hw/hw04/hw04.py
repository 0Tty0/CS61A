...
def balanced(m):
    ...
    if length(left(m))*total_weight(end(left(m)))==length(right(m))*total_weight(end(right(m))):
        if is_mobile(end(left(m))) and is_mobile(end(right(m))):
            if balanced(end(left(m)))==False or balanced(end(right(m)))==False:
                return False
        return True
    else:
        return False
...
def replace_leaf(t, find_value, replace_value):
    ...
    """
    labels =label(t)
    if is_leaf(t) and labels==find_value:
        return tree(replace_value)
    return tree(labels,[replace_leaf(node,find_value,replace_value) for node in branches(t)])
...
def has_path(t, phrase):
    ...
    if label(t)==phrase[0]:
        phrase=phrase[1:]
    return any([has_path(node,phrase) for node in branches(t)])
