...
def reverse_iter(lst):
    ...
    for i in range(0,lenth//2,1):
        lst[i],lst[lenth-i-1]=lst[lenth-i-1],lst[i]
    return lst
...
def distance(city_a, city_b):
    ...
    5.0
    """
    return sqrt(pow(city_a[1]-city_b[1], 2) + pow(city_a[2]-city_b[2], 2))
...
def closer_city(lat, lon, city_a, city_b):
    ...
    dis_a=distance(city_a,city)
    dis_b=distance(city_b,city)
    if dis_a<dis_b:
        return city_a[0]
    else:
        return city_b[0]
...
def add_chars(w1, w2):
    ...
    def is_in_w2(i,j):
        if i>=length2:return ''
        if w2[i]in w1[j::]:
            return is_in_w2(i+1,j+1)
        if w2[i] not in w1[j::]:
            return w2[i]+is_in_w2(i+1,j)
    return is_in_w2(0,0)
