...
def riffle(deck):
    ...
    """
    length = len(deck)
    return [ deck[(k%2)*(length//2)+(k)//2] for k in range(length)]
...
def add_trees(t1, t2):
    ...
    if is_leaf(t2):
        ...
        part1=fewer_branches
        for i in range(0,len(more_branches)-len(fewer_branches),1):
            part1+=[tree(0)]
        part2=more_branches
        return tree(label(t1)+label(t2),[add_trees(b1,b2) for b1,b2 in zip(part1,part2)])
